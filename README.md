...
### 3. Windows デスクトップアプリ (`WindowsApp` - NEW)
* **C# / .NET 8 WPF によるプレミアムデスクトップダッシュボード**:
  * Windows 11 の Sleek Dark Mode に最適化された、超美麗なメーターXAML UI。
* **完全なポータブル仕様 (インストール不要)**:
  * 必要な実行ライブラリを1ファイルにパッケージ化した「自己完結型・単一EXEファイル」形式でビルド。
  * `release/ESP32LoggerWin.exe` をコピーしてダブルクリックするだけで、他のPCでもランタイムのインストール不要で即座に動作。
* **柔軟なデータ保存先 & PPR設定管理**:
  * デフォルトのデータ保存先、および設定ファイルの配置先はexeと同一フォルダ。
  * 設定画面から直感的に「保存先フォルダ (SaveDirectory)」や「PPR (パルス/回転) 補正設定」をカスタマイズでき、`config.json` に永続保存。
* **トリプルネオンLEDインジケーター**:
  * BT接続状況（COMポート疎通）、GPS測位ステータス、アンテナ接続状況（衛星受信強度/データ整合性）の3つのステータスを、視認性の高い発光LEDランプでリアルタイムに表現。
* **別窓リアルタイムログモニター**:
  * メインウィンドウから独立したログウィンドウをワンタップで開閉可能。ESP32から受信した生のチェックサム付きデータを等幅フォントでリアルタイムにスクロール表示し、高度な動作解析やデバッグが可能。
...
### 3. Windowsアプリ側のログ（CSV）
PC側で接続時に自動でロギングが行われます。

* **保存タイミング**:
  * **記録中（リアルタイム）**: シリアルポート経由でデータを受信するたびに、指定された保存先フォルダのCSVファイルに即時アペンド（追記）されます。
  * **保存の確定**: アプリ上の「切断」ボタンを押した際、またはアプリを終了した際にファイルが安全にフラッシュされ、書き込みが完了します。
* **保存先フォルダーとパス**:
  * デフォルトでは実行ファイル（`ESP32LoggerWin.exe`）と同一フォルダに `log_yyyyMMdd.csv` として保存されます。
  * UIの「ログ保存フォルダ参照」ボタンから任意のフォルダに変更でき、設定ファイル `config.json` にパスが保存されるため次回起動時も自動的に引き継がれます。
* **保存フォーマット**:
  * Androidアプリ版のCSVと完全互換であり、1行目に列名ヘッダーを含み、パルス周期から換算された実際のRPM値（回転数）とJSTタイムスタンプを保存します。
  * **ヘッダー定義**: `JST時刻,raw_pulse_rpm,display_rpm,速度_kmh,緯度,経度,UTC時刻`
  * **データ例**: `2026-05-29 12:34:56.789,2000,4000,60.5,35.681234,139.767123,03:34:56`
...
│   │   └── build.gradle.kts
│   └── build.gradle.kts
└── WindowsApp/                 # Windows 11 連携アプリケーション (C# / WPF) (NEW)
    ├── ESP32LoggerWin.csproj   # ポータブル単一ファイルビルド設定
    ├── App.xaml / .xaml.cs     # アプリケーション開始定義
    ├── AppConfig.cs            # 設定永続化 (config.jsonの入出力)
    ├── MainWindow.xaml / .cs   # メインダッシュボード (メーター、接続、ログ保存)
    └── LogWindow.xaml / .cs    # 別ウィンドウ生ログデバッグモニター
```
...
### ③ Windows 側 (NEW)
1. **Bluetoothペアリングの準備**:
   * Windows 11 の設定から「Bluetooth とデバイス」を開き、「デバイスの追加」をクリック。
   * 発見された「`ESP32_Logger`」を選択し、PINコード **`1234`** を入力して接続（ペアリング）を完了します。
2. **COMポート番号の確認**:
   * Windowsの「コントロールパネル」＞「デバイスとプリンター」または「デバイスマネージャー」の「ポート (COM と LPT)」項目から、`ESP32_Logger` の**送信接続に割り当てられた COMポート番号**（例: `COM4`）を確認します。
3. **起動**:
   * ポータブル単一ファイルである **`release/ESP32LoggerWin.exe`** を起動します。（インストーラ不要でコピーのみで動作）。
4. **接続と設定**:
   * アプリ下部の「接続ポート」コンボボックスから、先ほど確認したCOMポート（例: `COM4`）を選択し、「**接続**」ボタンをクリックします。
   * 接続が確立すると「BT接続」ランプが鮮やかな緑になり、リアルタイムメーター（速度/RPM）の同期が開始されます。
   * 「ログ保存フォルダ」の「参照...」ボタンから、CSVログデータの出力先を自由に変更可能です。
...
### v1.4.1 — 2026-05-29 アプリアイコン刷新 (NEW)

| 対象 | 内容 |
| :--- | :--- |
| 🎨 共通 | **プレミアムアプリアイコンの導入**: 高精度なGPS/タコメーターを表現した美麗なネオン＆ダークの3Dグラスモルフィズム風アプリアイコンを新規に生成・刷新。 |
| 🤖 Android | **Androidアプリアイコン適用**: アプリ起動用の `ic_launcher` を新アイコンに完全統合。 |
| 💻 Windows | **Windowsアプリアイコン適用**: 単一ファイルEXEのビルド用およびメインウィンドウ（タスクバー・ウィンドウ左上）のアイコンとしてマルチサイズ対応の `app_icon.ico` を完全統合。 |

### v1.4.0 — 2026-05-29 Windows 11用ポータブルアプリ追加 (NEW)

| 対象 | 内容 |
| :--- | :--- |
| 💻 Windows | **WPFポータブルメーター追加**: C# (.NET 8 WPF) で自己完結型の単一ファイルダッシュボードを追加。 |
| 💻 Windows | **トリプルネオンLEDインジケーター**: Bluetooth接続状況、GPS測位状態、アンテナ接続状況をリアルタイム検知。 |
| 💻 Windows | **別窓デバッグログウィンドウ**: メインウィンドウと連動して生のチェックサム付きCSVパケットを等幅フォントでリアルタイム表示可能。 |
| 💻 Windows | **カスタマイズ可能なデータ保存先**: デフォルトはexeと同じフォルダ。UIの参照ダイアログから簡単に任意フォルダへ切り替え可能。 |
| 💻 Windows | **自動A-GPS送信**: 接続確立と同時にPCのローカル現在地・UTC高精度時刻を `$AGPS` パケットとして自動転送。 |
